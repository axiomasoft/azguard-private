...
«может ли пользователь действовать здесь?» — но это два разных механизма by design:

| | Context (эта страница) | [Entity scope](/ru/advanced/entity-scopes) |
|---|---|---|
| Время жизни | Runtime — резолвится на запрос, живёт его длительность | Persist — хранится как роль на модели |
...
Выбирайте **context**, когда нужен request-scoped переключатель «текущий workspace/tenant»,
меняющий применяемые права на весь остаток запроса. Выбирайте **entity scope**, когда нужна
персистентная привязка «этот пользователь — editor именно на Project A», которая переживает
запросы. Механизмы не сливаются и не заменяют друг друга — используйте оба вместе, если в
приложении есть и переключение tenant'ов, и роли на конкретных записях.
