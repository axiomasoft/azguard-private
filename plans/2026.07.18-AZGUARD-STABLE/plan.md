...
| Target Operator Models | GPT-5.6 Terra (implementation) · GPT-5.6 Luna (economy/read-only) · GPT-5.6 Sol (design/audit) — D33/D34 |
...
- Все незавершённые items P4/P5 исполняются только ChatGPT/Codex по обязательному
  `research/05-codex-execution-contract.md` (D34). Требуется свежая сессия после установки
  `task@swissknifeman`; Claude Code и ручной обход task-harness не являются fallback.
...
| P4 | Тест-углубление (ось корректности): docker БД-матрица, portability-ремедиация, race, mutation-ratchet | 6/12 | 🔴 Blocked |
