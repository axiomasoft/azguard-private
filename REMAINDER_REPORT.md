...
| T2 | 🟢 | **Закрыт** (план `2026.07.17-AZGUARD-TAILS`, P1.3) | `removeScopedRole($role, $entity, panelId=null)` теперь удаляет только any-panel строку — симметрично `assignScopedRole` (Вариант B, D10). Снос «везде» — новый метод `removeScopedRoleEverywhere()`. Breaking change, см. `packages/core/CHANGELOG.md`. |
| T3 | 🟢 | **Закрыт** (план `2026.07.17-AZGUARD-TAILS`, P2.1) | `EnumPermissionCatalogBuilder::build()` теперь логирует `Log::warning()` на missing enum-классе — паритетно `PolicyAbilityCatalogBuilder`. Тест на вызов лога в `tests/Unit/Registry/EnumPermissionCatalogBuilderTest.php`. |
| T4 | 🟢 | **Закрыт** (план `2026.07.17-AZGUARD-TAILS`, P2.2) | Wildcard-off ветка `filterAgainstCatalog()` теперь исключает ключи с `*` до проверки против dynamic-определений — паритетно wildcard-ON ветке (`str_contains($key, WILDCARD)`-гвард). Позитив+негатив тест в `tests/Unit/Registry/EffectivePermissionResolverTest.php`. |
| T5 | 🟢 | **Закрыт** (план `2026.07.17-AZGUARD-TAILS`, P2.3) | Explicit rollback-тест добавлен: `down()` мигрирует ПОСЛЕ создания строки с `scope_class = null`, на SQLite (тестовая БД проекта) падает `QueryException` («NOT NULL constraint failed») — задокументированное в докблоке поведение подтверждено ЭКСПЕРИМЕНТОМ, не предположением; докблок не потребовал правки. Тест в `tests/Feature/ScopeClassMigrationRollbackTest.php`. |
